...
function TextList({ values, onChange, label, addLabel, removeLabel }: { values: string[]; onChange: (next: string[]) => void; label: string; addLabel: string; removeLabel: string }) {
  return <div><div className="flex items-center justify-between gap-3"><span className="text-sm font-medium text-foreground">{label}</span><button type="button" onClick={() => onChange([...values, ''])} aria-label={`${addLabel}: ${label}`} className="text-sm font-medium text-[var(--lab-accent)] hover:underline">{addLabel}</button></div><div className="mt-2 space-y-2">{values.map((value, index) => <div key={`${label}-${index}`} className="flex gap-2"><input value={value} onChange={(event) => onChange(values.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} aria-label={`${label} ${index + 1}`} className={inputClass} /><button type="button" disabled={values.length === 1} onClick={() => onChange(values.filter((_, itemIndex) => itemIndex !== index))} aria-label={`${removeLabel}: ${label} ${index + 1}`} className="min-h-[44px] rounded-md border border-border px-3 text-sm text-muted-foreground disabled:opacity-40">{removeLabel}</button></div>)}</div></div>
}
