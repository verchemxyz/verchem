...
export default function RecordsPage() {
  ...
  useEffect(() => {
    let active = true
    void labFetch<{ records: LabRecordListItem[] }>(`/api/lab/orgs/${encodeURIComponent(org)}/records?limit=50`, { fallbackMessage: t.unknownError })
      .then((response) => { if (active) setRecords(response.records) })
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="lab-document overflow-hidden">
        {shown.length === 0 ? <p className="p-6 text-muted-foreground">{t.noRecords}</p> : <ul className="divide-y divide-border">{shown.map((record) => <li key={record.id}><Link href={`/lab/${org}/records/${record.id}`} className="flex flex-wrap items-center justify-between gap-3 p-4 hover:bg-muted"><div><p className="font-mono text-sm font-medium tabular-nums">{record.record_no}</p><p className="mt-1 text-sm text-muted-foreground">{record.template_name ?? record.template_key ?? t.template} · {formatLabDate(record.created_at)}</p></div><StatusChip state={record.state} outcome={record.outcome} /></Link></li>)}</ul>}
      </section>
    </div>
  )
}
