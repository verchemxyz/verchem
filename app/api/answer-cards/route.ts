 ...
 * - MOAT GATE: a card can only be saved if its Ed25519 JWS verifies. Since
 *   only the server holds the private key, this proves the card was generated by
 *   VerChem — a forged "VERIFIED" card cannot be persisted.
 * - aiverid taken from the session, never the client.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // — an attacker can't burn parse/Ed25519 CPU for free by spamming bad cards.
    ...
    // by this server (its private key) can be saved. Tampering with any signed
    // field breaks the JWS and is rejected here.
    ...
    const saved = await createAnswerCard({ aiverid: session.userId, card, is_public })
