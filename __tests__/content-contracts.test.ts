...
const verifiedCalculationPage = read('app/tools/verified-calculation/page.tsx')
const verifierPage = read('app/verify/page.tsx')
const deterministicCard = read('lib/answer-cards/deterministic-card.ts')
const browserVerifier = read('lib/answer-cards/browser-verifier.ts')
...
assert.match(homePage, /href="\/tools\/verified-calculation"/)
assert.match(homePage, /href="\/verify"/)
...
assert.match(toolsHubPage, /href: '\/tools\/verified-calculation'/)
assert.match(toolsHubPage, /href: '\/verify'/)
assert.match(toolsHubPage, /AIVerID sign-in is required/)
assert.doesNotMatch(`${homePage}\n${toolsHubPage}`, /\b(?:Ketcher|RDKit)\b/i)

assert.match(verifiedCalculationPage, /No AI or sign-in required/)
assert.match(verifierPage, /Verify a VerChem Artifact/)
assert.match(deterministicCard, /buildProvenanceEnvelope/)
assert.match(deterministicCard, /verchem-deterministic/)
assert.match(browserVerifier, /crypto\.subtle\.verify/)
assert.match(browserVerifier, /RFC 7638 thumbprint/)
