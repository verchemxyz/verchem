...
VerChem calculations use documented formulas and cite published references including NIST, CRC, IUPAC, and CODATA where applicable. VerChem is not certified by those organizations. Model scope and required inputs differ by calculator:
...
3. **Reference checks**: Only the test vectors explicitly named by a calculator's test suite have been compared with their cited values; no universal error bound is claimed.
4. **Research Use**: For publication-quality results, users should verify calculations independently.
5. **Temperature/Pressure**: Use only the conditions declared by the selected model; no global temperature or pressure is silently assumed.
...
`
  ...
  report += `\n## Validation\n`
  report += `This deterministic result uses the formula and references listed above. Verify that the declared model and inputs apply to the physical system; this report is not certification by a reference publisher.\n`
